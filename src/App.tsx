...
function ExerciseEditor({ value, onClose, onSaved }: { value: ExerciseRow | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <SidePanel title={value ? '编辑动作' : '新增动作'} subtitle="动作可被多个课程重复使用" onClose={onClose} footer={<div className="panel-actions"><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" type="submit" form="exercise-form" disabled={busy}>{busy ? '正在保存' : '保存动作'}</button></div>}>
    <form id="exercise-form" className="editor-form exercise-editor" onSubmit={submit}>{error && <div className="form-error">{error}</div>}
      <FormSection title="基本信息"><div className="form-grid"><Field label="动作名称" required><input value={form.name} onChange={(event) => update('name', event.target.value)} required /></Field><Field label="训练部位" required><select value={form.bodyPart} onChange={(event) => update('bodyPart', event.target.value)} required>{Array.from(new Set(['核心', '肩背', '下肢', '全身', form.bodyPart].filter(Boolean))).map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="难度" required><select value={form.level} onChange={(event) => update('level', event.target.value)} required>{Array.from(new Set(['基础', '进阶', '拉伸', form.level].filter(Boolean))).map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="器械"><input value={form.equipment} maxLength={80} onChange={event => update('equipment', event.target.value)} required /></Field></div></FormSection>
