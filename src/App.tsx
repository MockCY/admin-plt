...
import { api, apiBlob, downloadMedia, getToken, json, MAX_MEDIA_FILE_SIZE_BYTES, MAX_MEDIA_FILE_SIZE_LABEL, mediaUrl, setToken, uploadMedia } from './api'
...
function formatWatchDuration(value?: number) {
...
function PlansPage() {
  ...
  return <Page title="计划管理" description="从动作库编排每日训练，停用计划不会影响历史记录。" action={<button className="button primary" onClick={() => setEditing('new')}><Plus size={17} />新增计划</button>}>
    ...
    <TableSurface loading={loading} empty={!data?.items.length} emptyText="还没有训练计划" emptyHint="创建计划并安排每日训练动作。">
      <Table><thead><tr><th>计划</th><th>计划周期</th><th>训练天数</th><th>动作总数</th><th>单次时长</th><th>状态</th><th>更新时间</th><th><span className="sr-only">操作</span></th></tr></thead>
        <tbody>{data?.items.map((item) => { const days = item.days || []; return <tr key={item.id}><td><div className="content-cell"><MediaThumbnail src={item.coverImage} label={item.title} icon="image" /><span><strong>{item.title}</strong><small>{item.subtitle || item.description || '尚未填写计划副标题'}</small></span></div></td><td>{Math.max(item.cycleDays || days.length, days.length)} 天</td><td>{days.length} 天</td><td>{days.reduce((sum, day) => sum + (day.exercises || []).length, 0)} 个</td><td>{item.sessionMinutes || 0} 分钟</td><td><Badge status={item.active ? 'PUBLISHED' : 'ARCHIVED'} /></td><td>{formatDate(item.updatedAt)}</td><td><RowActions onEdit={() => setEditing(item)} onDelete={() => setDeleting(item)} /></td></tr> })}</tbody>
      </Table>
    </TableSurface>
...
function UserEditor({ value, onClose, onSaved }: { value: UserRow; onClose: () => void; onSaved: () => void }) {
  ...
  return <SidePanel title="编辑用户绑定" subtitle={`${displayName} · ID ${value.id}`} onClose={onClose} footer={<div className="panel-actions"><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" type="submit" form="user-form" disabled={busy || !phoneValid}>{busy ? <><LoaderCircle className="spin" size={17} />正在保存</> : '保存用户'}</button></div>}>
    <form id="user-form" className="editor-form" onSubmit={submit}>
      {error && <div className="form-error" role="alert">{error}</div>}
      <dl className="detail-list user-edit-summary"><div><dt>用户</dt><dd>{displayName}</dd></div><div><dt>内容记录</dt><dd>{value.workoutCount} 次</dd></div><div><dt>累计观看</dt><dd>{formatWatchDuration(value.totalSeconds)}</dd></div><div><dt>注册时间</dt><dd>{formatDate(value.createdAt)}</dd></div></dl>
...
function PlanEditor({ value, onClose, onSaved }: { value: PlanRow | null; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    cycleDays: Math.max(value?.cycleDays || initialDays.length, initialDays.length),
    description: value?.description || '', subtitle: value?.subtitle || '', coverImage: value?.coverImage || '',
    detailImage: value?.detailImage || '', homeImage: value?.homeImage || '',
  ...
  const [homeUploadState, setHomeUploadState] = useState<'idle' | 'uploading' | 'error'>('idle')
  const exercises = useResource<PageResult<ExerciseRow>>('/exercises?status=PUBLISHED&page=1&pageSize=500').data?.items || []
  const imageUploadPending = coverUploadState !== 'idle' || detailUploadState !== 'idle' || homeUploadState !== 'idle'
  const update = (key: string, next: unknown) => setForm((current) => ({ ...current, [key]: next }))
  ...
  const addDay = () => {
    const days = normalizeDays([...form.days, { dayNumber: form.days.length + 1, title: `第 ${form.days.length + 1} 天训练`, sortOrder: (form.days.length + 1) * 10, exercises: [] }])
    setForm((current) => ({ ...current, days, cycleDays: Math.max(current.cycleDays, days.length) }))
  }
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!Number.isInteger(form.cycleDays) || form.cycleDays < form.days.length) { setError(`计划周期不能少于训练天数（当前 ${form.days.length} 天）`); return }
    if (form.cycleDays > 365) { setError('计划周期不能超过 365 天'); return }
  ...
  return <SidePanel className="plan-editor-panel" wide title={value ? '编辑训练计划' : '新增训练计划'} subtitle="按训练日编排动作，保存后同步到小程序" onClose={onClose} footer={<div className="panel-actions"><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" form="plan-form" disabled={busy || imageUploadPending}>{busy ? '正在保存' : imageUploadPending ? '图片上传中' : '保存计划'}</button></div>}>
    <form id="plan-form" className="editor-form plan-editor" onSubmit={submit}>{error && <div className="form-error" role="alert">{error}</div>}<FormSection step={1} title="计划信息"><Field label="计划名称" required><input value={form.title} maxLength={80} onChange={(event) => update('title', event.target.value)} placeholder="例如 7天新手基础训练" required /></Field><Field label="一句话目标"><input value={form.subtitle} maxLength={160} onChange={(event) => update('subtitle', event.target.value)} placeholder="例如 从足部激活到核心稳定，逐步建立基础控制" /></Field><div className="form-grid"><Field label="计划周期（天）" required hint={`当前共 ${form.days.length} 个训练日，周期不能少于 ${form.days.length} 天`}><input type="number" min={form.days.length} max="365" value={form.cycleDays} aria-invalid={form.cycleDays < form.days.length} onChange={(event) => update('cycleDays', Number(event.target.value))} required /></Field><Field label="难度"><select value={form.level} onChange={(event) => update('level', event.target.value)}>{Array.from(new Set(['基础', '进阶', '挑战', '舒缓', value?.level].filter(Boolean))).map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="训练场景"><input value={form.trainingScene} maxLength={30} onChange={(event) => update('trainingScene', event.target.value)} placeholder="例如 居家核心床" /></Field><Field label="单次时长（分钟）"><input type="number" min="1" max="600" value={form.sessionMinutes} onChange={(event) => update('sessionMinutes', Number(event.target.value))} /></Field></div><Field label="计划介绍"><textarea rows={4} value={form.description} maxLength={300} onChange={(event) => update('description', event.target.value)} placeholder="说明计划适合人群、训练目标与节奏" /></Field></FormSection>
      <FormSection title="展示素材" description="首页训练卡、计划列表和计划详情可分别配置图片；首页图片未上传时自动使用列表图。"><div className="plan-media-grid"><MediaUploader label="首页展示图片" kind="image" value={form.homeImage} hint="建议横版 16:9，用于首页“当前训练计划”卡片" disabled={busy} onUploadStateChange={setHomeUploadState} onChange={(next) => update('homeImage', next)} /><MediaUploader label="列表展示图片" kind="image" value={form.coverImage} hint="建议竖版或 4:3，适合列表缩略图" disabled={busy} onUploadStateChange={setCoverUploadState} onChange={(next) => update('coverImage', next)} /><MediaUploader label="详情展示图片" kind="image" value={form.detailImage} hint="建议 16:9 横图，展示在计划详情顶部" disabled={busy} onUploadStateChange={setDetailUploadState} onChange={(next) => update('detailImage', next)} /></div></FormSection>
...
function MediaUploader({ label, kind, value, poster, durationSeconds, onChange, onDurationChange, hint, disabled = false, onUploadStateChange }: { hint?: string; label: string; kind: 'image' | 'video' | 'audio'; value: string; poster?: string; durationSeconds?: number; onChange: (value: string) => void; onDurationChange?: (value: number) => void; disabled?: boolean; onUploadStateChange?: (state: 'idle' | 'uploading' | 'error') => void }) {
  ...
  const upload = async (file?: File) => {
    ...
    if (file.size > MAX_MEDIA_FILE_SIZE_BYTES) {
      setError('')
      setUploadError(`文件过大，单个文件不能超过 ${MAX_MEDIA_FILE_SIZE_LABEL}`)
