...
const states:Record<string,string> = {WAITING:'等待上报',MOVING:'运动中',STILL:'在线静止',STANDBY:'上次状态：待机',OFFLINE:'离线',ERROR:'传感器异常',DISABLED:'已停用',ACTIVE:'进行中',COMPLETED:'已结束'}
const reasons:Record<string,string> = {IDLE:'停止运动满 3 分钟',STANDBY:'设备待机',INTERRUPTED:'设备重启或归属变更',UNBOUND:'用户解除绑定',ADMIN_UNBOUND:'管理员解除绑定',DISABLED:'管理员停用'}
...
export function SensorsPage() {
  ...
  const [query,setQuery]=useState(''),[search,setSearch]=useState(''),[state,setState]=useState('ALL'),[binding,setBinding]=useState('ALL'),[page,setPage]=useState(1)
  ...
  return <div className="page sensor-admin"><header className="page-header"><div><p className="eyebrow">设备运营</p><h1>传感器管理</h1></div><Refresh auto={auto} setAuto={setAuto} reload={reload} loading={loading}/></header><ContextFilter params={params}/>
    <form className="toolbar sensor-toolbar" onSubmit={e=>{e.preventDefault();setSearch(query);setPage(1)}}><label className="search-field"><Search size={18}/><input aria-label="搜索传感器" placeholder="编号、核心床 SN、用户或手机号" value={query} onChange={e=>setQuery(e.target.value)}/></label><select aria-label="传感器状态" value={state} onChange={e=>{setState(e.target.value);setPage(1)}}><option value="ALL">全部设备状态</option>{['MOVING','STILL','STANDBY','OFFLINE','ERROR','WAITING','DISABLED'].map(s=><option key={s} value={s}>{states[s]}</option>)}</select><select aria-label="传感器绑定状态" value={binding} onChange={e=>{setBinding(e.target.value);setPage(1)}}><option value="ALL">全部绑定状态</option><option value="BOUND">已绑定核心床</option><option value="UNBOUND">未绑定核心床</option></select><button className="button secondary" type="submit">搜索</button></form>
...
export function SensorWorkouts() {
  ...
  return <div className="page sensor-admin"><header className="page-header"><div><p className="eyebrow">训练数据</p><h1>设备训练记录</h1></div><Refresh auto={auto} setAuto={setAuto} reload={reload} loading={loading}/></header><ContextFilter params={context}/>
    <form className="toolbar sensor-toolbar" onSubmit={submit}><label className="search-field"><Search size={18}/><input aria-label="搜索设备训练记录" placeholder="用户、手机号、核心床 SN 或传感器" value={query} onChange={e=>setQuery(e.target.value)}/></label><select aria-label="训练状态" value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="ALL">全部训练状态</option><option value="ACTIVE">进行中</option><option value="COMPLETED">已结束</option></select><label className="date-filter"><span>开始日期</span><input aria-label="开始日期" type="date" value={from} max={to || undefined} onChange={e=>{setFrom(e.target.value);setPage(1)}}/></label><label className="date-filter"><span>结束日期</span><input aria-label="结束日期" type="date" value={to} min={from || undefined} onChange={e=>{setTo(e.target.value);setPage(1)}}/></label><button className="button secondary" type="submit">搜索</button><button className="button ghost" type="button" onClick={()=>{setQuery('');setSearch('');setStatus('ALL');setFrom('');setTo('');setPage(1)}}>重置</button></form>
    {data && !error && <div className="sensor-summary" aria-label="筛选结果汇总"><span>训练记录 <strong>{data.total}</strong></span><span>进行中 <strong>{data.summary.activeSessions}</strong></span><span>训练时长 <strong>{duration(data.summary.durationMs)}</strong></span><span>往返次数 <strong>{data.summary.repetitions}</strong></span><small>北京时间</small></div>}
    <Result loading={loading} error={error} empty={!data?.items.length} retry={reload}><section className="table-surface"><div className="table-scroll"><table><thead><tr><th>用户</th><th>核心床 / 传感器</th><th>开始时间</th><th>最后动作时间</th><th>训练时长</th><th>往返次数</th><th>状态</th><th>结束原因</th><th>详情</th></tr></thead><tbody>{data?.items.map(item=><tr key={item.id}><td><div className="cell-stack"><strong>{item.userName || `用户 ${item.userId}`}</strong><small>{item.userPhone || `ID ${item.userId}`}</small></div></td><td><div className="cell-stack"><span>{item.bedSn || `核心床 ${item.bedId}`}</span><small>{item.deviceCode}</small></div></td><td>{time(item.startedAt)}</td><td>{time(item.lastMotionAt)}</td><td>{duration(item.durationMs)}</td><td>{item.repetitionCount}</td><td>{stateBadge(item.status)}</td><td>{item.endReason ? reasons[item.endReason] || item.endReason : '未结束'}</td><td><button className="icon-button" title="训练详情" aria-label={`查看训练 ${item.id}`} onClick={()=>setSelected(item)}><Eye size={17}/></button></td></tr>)}</tbody></table></div></section></Result>{data && !error && <Pager data={data} onPage={setPage}/>}
...
function SessionDetail({id,onClose}:{id:number;onClose:()=>void}) {
  ...
  return <Drawer title={`设备训练 #${id}`} subtitle={current?.deviceCode} onClose={onClose}><Result loading={loading} error={error} empty={!current} retry={()=>setVersion(v=>v+1)}>{current && <><dl className="detail-list">{[['用户',`${current.userName || '用户'} · ID ${current.userId}`],['核心床',current.bedSn || String(current.bedId)],['传感器',current.deviceId],['首次动作',time(current.startedAt)],['最后动作',time(current.lastMotionAt)],['结束时间',time(current.endedAt)],['训练时长',duration(current.durationMs)],['往返次数',String(current.repetitionCount)],['起始累计计数',String(current.startCount)],['结束累计计数',String(current.endCount)],['状态',states[current.status]],['结束原因',current.endReason ? reasons[current.endReason] || current.endReason : '未结束']].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><a className="button secondary" href={`#/sensors?bedId=${current.bedId}&label=${encodeURIComponent(current.bedSn || '核心床 '+current.bedId)}`}><Activity size={16}/>查看核心床当前传感器</a></>}</Result></Drawer>
}
