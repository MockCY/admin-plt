...
const calories = (value?:number)=>typeof value==='number' && Number.isFinite(value) && value>=0 ? `${value.toFixed(1)} kcal` : '暂无数据'
const calorieWeight = (item:Session)=>typeof item.calorieWeightKg==='number' && Number.isFinite(item.calorieWeightKg) && item.calorieWeightKg>0 ? `${item.calorieWeightKg.toFixed(1)} kg${item.calorieWeightDefaulted===true ? '（默认参考体重）' : item.calorieWeightDefaulted===false ? '（用户当前体重）' : ''}` : '暂无数据'
const calorieEstimateNote = '按用户当前体重、有效训练时长与往返频率估算；未填写体重时按 60 kg。更新体重后，历史消耗会重新估算。'
...
export function SensorWorkouts() {
  ...
  return <div className="page sensor-admin device-workouts"><header className="page-header"><div><h1>设备训练记录</h1></div><Refresh auto={auto} setAuto={setAuto} reload={reload} loading={loading}/></header><ContextFilter params={context}/>
    ...
    {data && !error && <div className="sensor-summary" aria-label="筛选结果汇总"><span>训练记录 <strong>{data.total}</strong></span><span>进行中 <strong>{data.summary.activeSessions}</strong></span><span>有效时长 <strong>{duration(data.summary.durationMs)}</strong></span><span>往返次数 <strong>{data.summary.repetitions}</strong></span><span>累计消耗（估算）<strong>{calories(data.summary.estimatedCalories)}</strong></span><span>筛选内今日消耗（估算）<strong>{calories(data.summary.todayEstimatedCalories)}</strong></span><small>{calorieEstimateNote}<br/>今日按北京时间训练开始日统计；日期筛选不含今日时为 0。</small></div>}
    <Result loading={loading} error={error} empty={false} retry={reload}><section className="workout-results" aria-label="设备训练列表"><div className="table-scroll"><table><thead><tr><th>用户</th><th>核心床 / 传感器</th><th>开始时间</th><th>最后动作时间</th><th>有效时长</th><th>往返次数</th><th>消耗（估算）</th><th>状态</th><th>结束原因</th><th>详情</th></tr></thead><tbody>{data?.items.map(item=><tr key={item.id}><td><div className="cell-stack"><strong>{item.userName || (item.userId ? `用户 ${item.userId}` : '归属待确认')}</strong><small>{item.userPhone || (item.userId ? `ID ${item.userId}` : '离线记录待核对')}</small></div></td><td><div className="cell-stack"><span>{item.bedSn || (item.bedId ? `核心床 ${item.bedId}` : '核心床待确认')}</span><small>{item.deviceCode}</small></div></td><td>{sessionTime(item.startedAt,item)}</td><td>{time(item.lastMotionAt)}</td><td>{duration(item.durationMs)}</td><td>{item.repetitionCount}</td><td>{calories(item.estimatedCalories)}</td><td>{stateBadge(sessionState(item))}</td><td>{item.endReason ? reasons[item.endReason] || item.endReason : '未结束'}</td><td><button className="icon-button" title="训练详情" aria-label={`查看训练 ${item.id}`} onClick={()=>setSelected(item)}><Eye size={17}/></button></td></tr>)}</tbody></table></div>
