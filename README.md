...
训练时长按首次动作至最后动作计算，连续 180 秒无动作结束本次训练，末尾等待不计入时长。日期筛选及显示使用北京时间。
此功能使用已有传感器表，需配套更新后端；不需要新增 SQL。

本地模拟预览（Node.js 22，在本目录执行）：

```text
node scripts/preview-sensors.mjs
```

访问 `http://127.0.0.1:5195/#/sensors`。该预览使用模拟接口，不连接业务后端。
`scripts/check-sensor-admin.mjs` 使用相邻 `flex-air-prototype` 的 Playwright 和本机 Edge，覆盖桌面/手机布局、筛选、详情及管理操作；仅供当前工作区联调。
